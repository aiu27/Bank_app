...
const calcDisplaySummary = function (acc) {
  ...
  const interest2 = acc.movements
    .filter(mov => mov > 0)
    .map(deposit => (deposit * acc.interestRate) / 100)
    .filter(int => int >= 1)
    .reduce((acc, mov) => acc + mov, 0);

  labelSumInterest.textContent = `${interest2}`;
};
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // dogsJulia.slice(1,-2);
  const together = correctJulia.concat(dogsKate);

  together.forEach(function (dog, i) {
    if (dog >= 3) {
...
// same result using other methods
...
const numDeposit1000second = accounts
  .flatMap(acc => acc.movements)
  .reduce((count, cur) => (cur >= 1000 ? ++count : count), 0);
console.log(numDeposit1000second);
...
console.log(
  `Sara's dog is eating too ${
    sarasDog.curFood < sarasDog.recommendedFood ? 'little' : 'much'
  }`
);
...
console.log(dogs.some(okRange));

// 7
const dogOK = dogs.filter(okRange);

console.log(dogOK);

// 8

// const recFoodinOrder = dogs
//   .map(dog => dog.recommendedFood)
//   .sort((a, b) => a - b);
// console.log(recFoodinOrder);
